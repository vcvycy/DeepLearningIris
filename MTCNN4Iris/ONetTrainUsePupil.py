...
if __name__ == "__main__":
    # 运行目录
    experiment_dir = "experiments/onet_with_proposal"
    ...
    while cur_step< config.total_training_steps:
        if cur_step > 2000:
            learning_rate = 1e-5
        batch = train_data.getBatchForONetWithPupil(config.batch_size,config.pos_region_each_image, config.neg_region_each_image)
