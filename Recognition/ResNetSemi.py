...
class ResNet:
    ...
    def train(self, batch_input, batch_output, learning_rate):
        _, loss,summary_val , global_step,eweight = self.sess.run([self.optimizer, self.loss,self.all_summary,self.global_step,self.eweight],
    ...
    def  trainWithClassification(self, batch_input, batch_output,batch_output_ont_hot, learning_rate,
                                 ...
                                 batch_all_weight=1,
                                 batch_hard_weight=0):
        _, cls_accu,loss,summary_val , global_step, l1,l2,l3,l4,eweight = self.sess.run([self.optimizer,
                                                           self.accuracy,
                                                           self.loss,
                                                           self.all_summary,
                                                           self.global_step,
        ...
        print("[*]accu = %s loss = %s  %s  %s  %s weight=%s" %(cls_accu,l1, l2, l3, l4, eweight[0][:10]))
        self.writer.add_summary(summary_val,global_step)
        return loss, global_step
